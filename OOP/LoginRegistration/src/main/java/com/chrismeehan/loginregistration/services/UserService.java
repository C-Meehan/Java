package com.chrismeehan.loginregistration.services;
...
import com.chrismeehan.loginregistration.repositories.UserRepository;
...
public class UserService {
	...
	public User register(User createdUser, BindingResult result) {
		if (!createdUser.getName().matches("[a-zA-Z]+")) {
			result.rejectValue("name", "Invalid", "User name must only contain letters");
			
		}
		if (!createdUser.getPassword().matches("^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@$!%*?&])[A-Za-z\\d@$!%*?&]{8,}$")) {
			result.rejectValue("password", "Invalid", "Password must contain at least 1 lowercase and uppercase letter. Must also contain at least 1 numbber and special character (@$!%*?&)");
